    ...
    for chars, info, other in lexed:
        if "single function" in info or info in (
            ...
            if isinstance(other, list):
                ...
                ret += info + ":\n"
                ret += auto_explain(
                    t, index + indent + 1, comment_indent + 1, False, len(code) - len("".join(x[0] for x in t)) - index - 1
                )
                ...
                if f:
                    ...
                    ret += auto_explain(
                        f, index + indent + 1, comment_indent + 1, False, len(code) - len("".join(x[0] for x in f)) - index - 1
                    )
